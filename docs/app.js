document.getElementById("recipe-form").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    // 豆の量を計算
    if (recipe.beans_amount && recipe.amount) {
      const beansDiv = document.createElement("div");
      beansDiv.innerHTML = `<strong>豆の量: ${recipe.beans_amount} g</strong>`;
      resultDiv.appendChild(beansDiv);
    }
    ...
    resultDiv.appendChild(startBtn);

    startBtn.addEventListener("click", async () => {
      startBtn.disabled = true; // 2回押し防止
          ...
          await new Promise(resolve => {
            const intervalId = setInterval(() => {
              ...
              if (elapsed >= step.duration_sec) {
                ...
                timerSpan.textContent = `Count: ${step.duration_sec.toFixed(1)}s`;
                resolve();
              }
            }, 100);
          });
